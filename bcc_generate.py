...
def generate_bcc_positions(box_size, num_particles):
    """
    Generate BCC lattice positions within a 3D box.
    ...
    for x in range(num_cells):
        for y in range(num_cells):
            for z in range(num_cells):
                # Corner atom
                positions.append([x * lattice_constant, y * lattice_constant, z * lattice_constant])
                # Body-centered atom
                positions.append([(x + 0.5) * lattice_constant, (y + 0.5) * lattice_constant, (z + 0.5) * lattice_constant])
